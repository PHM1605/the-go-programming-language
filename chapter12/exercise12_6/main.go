...
func encode(buf *bytes.Buffer, v reflect.Value, indent int) error {
	...
	case reflect.Array, reflect.Slice:
		buf.WriteByte('(')
		// NEW: insert newline for 1st element
		if v.Len() > 0 {
			buf.WriteByte('\n')
		}
		for i := 0; i < v.Len(); i++ {
	...
	case reflect.Struct:
		...
		// NEW: is that the first Field we are inserting?
		first := true
		...
		for i := 0; i < v.NumField(); i++ {
			...
			// if not first field then we put "newline" and "indent"
			if !first {
				buf.WriteByte('\n')
				writeIndent(buf, indent+1)
			}
	...
	case reflect.Map:
		...
		for i, key := range keys { // key: reflect.Value of "Dr. Strangelove"
			...
			buf.WriteByte(')')
			// newline at the end of each line EXCEPT the last
			if i != len(keys)-1 {
				buf.WriteByte('\n')
			}
		}
