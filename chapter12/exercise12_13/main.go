...
// NEW: TAG to note what to do with this Field
// `sexpr:"-"` tag means "We don't want to encode this field"
type Movie struct {
...
func encode(buf *bytes.Buffer, v reflect.Value) error {
	...
	case reflect.Struct:
		buf.WriteByte('(')
		// are we writing later than 1st field? => no need ' ' prior
		first := true
		for i := 0; i < v.NumField(); i++ {
			field := v.Type().Field(i)     // metadata of Field i
			name := field.Tag.Get("sexpr") // get tag information
			// exclude field with tag "-" or take its original-name if no tag
			switch name {
			case "-":
				continue
			case "":
...
// decode from a lexer to object
// v: reflect.Value of an addressible "Movie" object initially
// later: reflect.Value of the "value" (2nd) part of a struct's entry e.g. Slice
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Struct: // ((name1 value1), (name2 value2), ...)
		// collect the Fields in struct to be decoded (with shorted key per in "tag")
		fields := make(map[string]reflect.Value)
		for i := 0; i < v.NumField(); i++ {
			field := v.Type().Field(i)     // metadata of Field i
			name := field.Tag.Get("sexpr") // effective name of that field
			switch name {
			case "-": // field with skipping tag i.e. "Color" field
				continue
			case "": // field with no tag e.g. "Dummy"
				name = field.Name
			}
			fields[name] = v.Field(i) // map from effective-name or original-name to it's value field (which will be assigned later)
		}
		...
		for !endList(lex) {
			...
			lex.next()         // '('
			// NEW: we update the field that we mapped above
			field, ok := fields[name]
			if !ok {
				panic(fmt.Sprintf("unknown field %q", name))
			}
			read(lex, field)
			lex.consume(')') // pop out ')' char (of each entry) and move to the next token
		}
