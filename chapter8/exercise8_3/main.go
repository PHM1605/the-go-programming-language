...
func main() {
	...
	// NEW: we close only the Write side of the connection (branch goroutine still running)
	tcpConn := conn.(*net.TCPConn) // check Inteface, copy into a new Interface with that value
	tcpConn.CloseWrite()           // server.Scan() will see EOF, hence done Echoing then handle EOF
